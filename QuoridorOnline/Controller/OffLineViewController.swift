...
class OffLineViewController: UIViewController, UIDragInteractionDelegate, UIDropInteractionDelegate, GADInterstitialDelegate{
    ...
    override func viewDidLoad() {
        ...
        // ターンプレイヤーの現在地以外のボタンは無効
        for button in buttons{
            if button.tag == nowTurn.nowPlace{
    ...
    func showWinner(){
        ...
        if nowTurn.nowPlace == p1.nowPlace{
            button.setImage(UIImage(named: "p1win"), for: .normal)
        }else{
            button.setImage(UIImage(named: "p2win"), for: .normal)
        }
...
//        let interstitial = GADInterstitial(adUnitID: "ca-app-pub-1391275810267480/2776440371")
    ...
    @IBAction func back(_ sender: Any) {
        let alert = UIAlertController(title: "タイトルへ戻ります", message: "進行状況は破棄されます", preferredStyle: .alert)
    ...
    @IBAction func a1Action(_ sender: Any) {
        highlightCell(button: sender as! UIButton)
    }
